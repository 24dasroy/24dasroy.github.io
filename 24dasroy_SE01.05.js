...
function setup() {
  ...
  shapeSelector.attribute("title", "Select Shape");
  ...
  colorPicker.input(() => {
    // Update currentColor with the value of the color picker
...
function drawShape() {
  ...
  switch (shapeSelector.value()) {
    ...
    case "diamond":
      ...
      break;
      case "heart": // Add this case for handling the "heart" shape
...
function drawHeart(x, y, size) {
  beginShape();
  vertex(x, y - size / 2);
  bezierVertex(x + size / 2, y - size, x + size, y - size / 2, x, y + size / 2); // Top arc of heart
  bezierVertex(x - size, y - size / 2, x - size / 2, y - size, x, y - size / 2); // Bottom arc of heart
  endShape(CLOSE);
}
